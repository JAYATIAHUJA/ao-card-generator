...
void main(){float a=uResolution.x/max(uResolution.y,1.);vec2 uv=(vUv-.5)*vec2(a,1.)+.5;float n=abs(noise(vec3(uv*1.8,uTime*.5)));float gridScale=47.;vec2 cell=fract(uv*gridScale)-.5;float radius=clamp(pow(n,7.)+0.1,0.,1.)*.5;float aa=fwidth(length(cell))+.002;float dotMask=1.-smoothstep(radius-aa,radius+aa,length(cell));color=vec4(vec3(dotMask),dotMask*.34);}`;
...
export function ChromaticWaves({ visible = true }: { visible?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    // A dim, slow dot grid: 1x DPR and ~30fps read the same while roughly
    // quartering the GPU work of the old 2x DPR every-frame loop.
    const renderer = new Renderer({ dpr: Math.min(devicePixelRatio || 1, 1.5), alpha: true });
    ...
    const start = () => { if (running || document.hidden) return; running = true; frame = requestAnimationFrame(tick); };
    const stop = () => { running = false; cancelAnimationFrame(frame); };
    const handleVisibility = () => { if (document.hidden) stop(); else start(); };
    document.addEventListener("visibilitychange", handleVisibility);
    start();
    return () => { document.removeEventListener("visibilitychange", handleVisibility); stop(); observer.disconnect(); gl.canvas.remove(); };
  }, []);
